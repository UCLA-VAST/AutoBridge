...
{
  {
    ...
    float *dev_A = (float *)malloc((20) * (20) * sizeof(float));
    float *dev_L = (float *)malloc((19) * (20) * sizeof(float));
    float *dev_U = (float *)malloc((20) * (20) * sizeof(float));

    // Initialize host buffers
    memcpy(dev_A, A, (20) * (20) * sizeof(float));
    memcpy(dev_L, L, (19) * (20) * sizeof(float));
    memcpy(dev_U, U, (20) * (20) * sizeof(float));
    ...
    for (int i = 0; i < 1; i++) {
      float *buffer_A_tmp = (float *)malloc((20) * (20) * sizeof(float));
      buffer_A.push_back(buffer_A_tmp);
    }
    for (int i = 0; i < 1; i++) {
      float *buffer_L_tmp = (float *)malloc((19) * (20) * sizeof(float));
      buffer_L.push_back(buffer_L_tmp);
    }
    for (int i = 0; i < 1; i++) {
      float *buffer_U_tmp = (float *)malloc((20) * (20) * sizeof(float));
      buffer_U.push_back(buffer_U_tmp);
    }

    for (int i = 0; i < 1; i++) {
      memcpy(buffer_A[i], dev_A, (20) * (20) * sizeof(float));
    }

    for (int i = 0; i < 1; i++) {
      memcpy(buffer_L[i], dev_L, (19) * (20) * sizeof(float));
    }

    for (int i = 0; i < 1; i++) {
      memcpy(buffer_U[i], dev_U, (20) * (20) * sizeof(float));
    }
    ...
    for (int i = 0; i < 1; i++) {
      memcpy(dev_L, buffer_L[i], (19) * (20) * sizeof(float));
    }

    for (int i = 0; i < 1; i++) {
      memcpy(dev_U, buffer_U[i], (20) * (20) * sizeof(float));
    }

    // Restore data from host buffers
    memcpy(L, dev_L, (19) * (20) * sizeof(float));
    memcpy(U, dev_U, (20) * (20) * sizeof(float));
