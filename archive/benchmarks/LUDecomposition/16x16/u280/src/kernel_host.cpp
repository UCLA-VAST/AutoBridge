...
{
  {
    ...
    float *dev_L = (float *)malloc((15) * (16) * sizeof(float));
    float *dev_U = (float *)malloc((16) * (16) * sizeof(float));

    // Initialize host buffers
    memcpy(dev_A, A, (16) * (16) * sizeof(float));
    memcpy(dev_L, L, (15) * (16) * sizeof(float));
    memcpy(dev_U, U, (16) * (16) * sizeof(float));
    ...
    for (int i = 0; i < 1; i++) {
      float *buffer_A_tmp = (float *)malloc((16) * (16) * sizeof(float));
      buffer_A.push_back(buffer_A_tmp);
    }
    for (int i = 0; i < 1; i++) {
      float *buffer_L_tmp = (float *)malloc((15) * (16) * sizeof(float));
      buffer_L.push_back(buffer_L_tmp);
    }
    for (int i = 0; i < 1; i++) {
      float *buffer_U_tmp = (float *)malloc((16) * (16) * sizeof(float));
    ...
    for (int i = 0; i < 1; i++) {
      memcpy(buffer_L[i], dev_L, (15) * (16) * sizeof(float));
    }

    for (int i = 0; i < 1; i++) {
      memcpy(buffer_U[i], dev_U, (16) * (16) * sizeof(float));
    }
    ...
    for (int i = 0; i < 1; i++) {
      memcpy(dev_L, buffer_L[i], (15) * (16) * sizeof(float));
    }

    for (int i = 0; i < 1; i++) {
      memcpy(dev_U, buffer_U[i], (16) * (16) * sizeof(float));
    }

    // Restore data from host buffers
    memcpy(L, dev_L, (15) * (16) * sizeof(float));
    memcpy(U, dev_U, (16) * (16) * sizeof(float));
