...
#pragma HLS INLINE OFF
  ...
  // io_L3
  // io_L2
  for (ap_uint<6> c1 = 0; c1 <= 23; c1 += 1) {
    // io_L1
    // pe
    for (ap_uint<6> c2 = 0; c2 <= 23; c2 += 1) {
    #pragma HLS PIPELINE II=1
      // access_coalesce
    {
      float fifo_data;
      fifo_data = A[c1 + 24*c2];
...
#pragma HLS INLINE
  ...
  for (ap_uint<6> c1 = p1; c1 <= 23; c1 += 1) {
    // io_L1
    if (c1 == p1) {
      for (ap_uint<6> c2 = 0; c2 <= 23; c2 += 1) {
      #pragma HLS PIPELINE II=1
...
#pragma HLS INLINE OFF
  ...
  // array
  // pe
  for (ap_uint<6> c2 = p0; c2 <= 23; c2 += 1) {
  #pragma HLS PIPELINE II=1
    {
      if (p0 == 0) {
...
#pragma HLS INLINE
  ...
  for (ap_uint<6> c1 = max(p0 + 1, p1); c1 <= 23; c1 += 1) {
  #pragma HLS PIPELINE II=1
    // io_L1
    if (c1 == p1) {
      // access_coalesce
...
#pragma HLS INLINE OFF
  ...
  // array
  // io_L3
  for (ap_uint<6> c0 = p0; c0 <= 22; c0 += 1) {
    // io_L2
    if (c0 == p0) {
      for (ap_uint<6> c1 = p0 + 1; c1 <= 23; c1 += 1) {
      #pragma HLS PIPELINE II=1
        // io_L1
        // access_coalesce
        float fifo_data;
      ...
      for (ap_uint<6> c1 = c0 + 1; c1 <= 23; c1 += 1) {
      #pragma HLS PIPELINE II=1
        // io_L1
        // access_coalesce
        float fifo_data;
...
#pragma HLS INLINE OFF
  ...
  // array
  // io_L3
  for (ap_uint<6> c0 = p0; c0 <= 23; c0 += 1) {
    // io_L2
    if (c0 == p0) {
      // io_L1
      // access_coalesce
