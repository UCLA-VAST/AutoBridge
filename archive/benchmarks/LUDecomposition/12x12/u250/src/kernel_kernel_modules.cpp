...
#pragma HLS INLINE
  ...
  for (int c1 = max(p0 + 1, p1); c1 <= 11; c1 += 1) {
    // io_L1
    if (c1 == p1) {
      // access_coalesce
      // hls_pipeline
...
#pragma HLS INLINE OFF
  ...
  // array
  // io_L3
  for (int c0 = p0; c0 <= 10; c0 += 1) {
    // io_L2
    if (c0 == p0) {
      for (int c1 = p0 + 1; c1 <= 11; c1 += 1) {
        // io_L1
        // access_coalesce
        // hls_pipeline
        float fifo_data;
