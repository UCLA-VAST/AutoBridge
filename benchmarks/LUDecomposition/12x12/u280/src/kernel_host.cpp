...
{
  {
    ...
    float *dev_A = (float *)malloc((12) * (12) * sizeof(float));
    float *dev_L = (float *)malloc((11) * (12) * sizeof(float));
    float *dev_U = (float *)malloc((12) * (12) * sizeof(float));

    // Initialize host buffers
    memcpy(dev_A, A, (12) * (12) * sizeof(float));
    memcpy(dev_L, L, (11) * (12) * sizeof(float));
    memcpy(dev_U, U, (12) * (12) * sizeof(float));
    ...
    for (int i = 0; i < 1; i++) {
      float *buffer_A_tmp = (float *)malloc((12) * (12) * sizeof(float));
      buffer_A.push_back(buffer_A_tmp);
    }
    for (int i = 0; i < 1; i++) {
      float *buffer_L_tmp = (float *)malloc((11) * (12) * sizeof(float));
      buffer_L.push_back(buffer_L_tmp);
    }
    for (int i = 0; i < 1; i++) {
      float *buffer_U_tmp = (float *)malloc((12) * (12) * sizeof(float));
      buffer_U.push_back(buffer_U_tmp);
    }

    for (int i = 0; i < 1; i++) {
      memcpy(buffer_A[i], dev_A, (12) * (12) * sizeof(float));
    }

    for (int i = 0; i < 1; i++) {
      memcpy(buffer_L[i], dev_L, (11) * (12) * sizeof(float));
    }

    for (int i = 0; i < 1; i++) {
      memcpy(buffer_U[i], dev_U, (12) * (12) * sizeof(float));
    }
    ...
    for (int i = 0; i < 1; i++) {
      memcpy(dev_L, buffer_L[i], (11) * (12) * sizeof(float));
    }

    for (int i = 0; i < 1; i++) {
      memcpy(dev_U, buffer_U[i], (12) * (12) * sizeof(float));
    }

    // Restore data from host buffers
    memcpy(L, dev_L, (11) * (12) * sizeof(float));
    memcpy(U, dev_U, (12) * (12) * sizeof(float));
