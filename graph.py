...
##########################################################
  ...
  def generateConstraint_2D(self):
    ...
    self.constraintModules(tcl)

    # self.constraintModulesSLRLevel(tcl)

    self.constraintEdges(tcl)
  ...
  def generateConstraint(self):
    ...
    for loc, vs in assignment.items():
      ...
      tcl.write(f'add_cells_to_pblock [get_pblocks pblock_dynamic_SLR{loc}] [get_cells -hierarchical -regexp {{\n')
      for v in vs:
        tcl.write(f'\t(.*/)?{v}\n')
      tcl.write('}] -clear_locs \n')
