...
class Graph:
  ...
  def __init__(self, formator):
    ...
    self.generateConstraint_U250_2D()
  ...
  def generateConstraint_U250_2D(self):
    ...
    for y in range(self.formator.SLR_CNT):
      ...
      tcl.write(f'''
startgroup
create_pblock pblock_{0}_{y}
resize_pblock pblock_{0}_{y} -add CLOCKREGION_X0Y{0+y*NUM_PER_SLR_HORIZONTAL}:CLOCKREGION_X3Y{3+y*NUM_PER_SLR_HORIZONTAL}
endgroup

startgroup
create_pblock pblock_{1}_{y}
resize_pblock pblock_{1}_{y} -add CLOCKREGION_X5Y{0+y*NUM_PER_SLR_HORIZONTAL}:CLOCKREGION_X6Y{3+y*NUM_PER_SLR_HORIZONTAL}
endgroup

''')
    assignment_v = d = defaultdict(lambda: defaultdict(list))
    for x in range(self.formator.column):
      for y in range(self.formator.SLR_CNT):
        for v in self.vertices.values():
          if (v.slr_loc_2d[0] == x and v.slr_loc_2d[1] == y):
            assignment_v[x][y].append(v.name)

    # kernels assign to half-SLR granularity
    for x in range(self.formator.column):
      for y in range(self.formator.SLR_CNT):
        names = assignment_v[x][y]
        
        # the command cannot take empty inputs
        if (len(names) == 0):
          continue

        tcl.write(f'add_cells_to_pblock [get_pblocks pblock_{x}_{y}] [get_cells -hierarchical -regex {{\n')
        for v in names:
          tcl.write(f'\t(.*/)?{v}\n')
        tcl.write('}]\n')

    assignment_e = d = defaultdict(lambda: defaultdict(list))
    for x in range(self.formator.column):
      for y in range(self.formator.SLR_CNT):
        for e in self.edges.values():
          if (e.dst.slr_loc_2d[0] == x and e.dst.slr_loc_2d[1] == y):
            if (e.mark):
              assignment_e[x][y].append(f'{e.name}/*fifo_unit')
            else :
              assignment_e[x][y].append(f'{e.name}')
    # edges only assign to SLR granularity
    for x in range(self.formator.column):
      for y in range(self.formator.SLR_CNT):
        names = assignment_e[x][y]
  ...
  def addRelayStation(self, node):
    ...
    step = abs(e.src.slr_loc_2d[0] - e.dst.slr_loc_2d[0]) + \
           abs(e.src.slr_loc_2d[1] - e.dst.slr_loc_2d[1])
    if (step):
